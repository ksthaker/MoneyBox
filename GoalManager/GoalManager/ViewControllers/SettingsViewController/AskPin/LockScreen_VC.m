...
@interface LockScreen_VC ()

@end



@implementation LockScreen_VC
...
- (void)viewDidLoad
{
    ...
    self.lbl_Heading.text = NSLocalizedString(@"Money Box Protected", nil);
    self.lbl_SubHeading.text = NSLocalizedString(@"Please enter your passcode to unlock", nil);
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if ([self.str_FromWhere isEqualToString:@"FromSettings"]) {
...
-(void) Hide_Lock_Screen
{
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        if ([self.str_FromWhere isEqualToString:@"FromSettings"]) {
            [self.obj_Setting_VC action_PasscodeOff];
        }
        [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent];
    }];
}
