...
- (void)viewDidLoad
{
    ...
    self.lbl_SubHeading.text = NSLocalizedString(@"To enable the Passcode Lock please choose a 4-digit code", nil);
...
- (void)handleSwipe:(UISwipeGestureRecognizer *)swipe
{
    ...
    {
        [Localytics tagEvent:@"Passcode back"];
        [Flurry logEvent:@"Passcode back"];
        
        [FIRAnalytics logEventWithName:@"Passcode_back"
                            parameters:nil];
...
-(IBAction) action_Enter_Pin:(UIButton *)sender
{
    ...
    if(str_PinCode.length==4){
        ConfirmPin_VC *obj_ConfirmPin_VC =[self.storyboard instantiateViewControllerWithIdentifier:@"ConfirmPin_VC"];
        obj_ConfirmPin_VC.str_PinNumber = str_PinCode;
        if ([self.str_FromWhere isEqualToString:@"FromTouchID"]) {
            obj_ConfirmPin_VC.str_FromWhere = @"FromTouchID";
        }
        [self.navigationController pushViewController:obj_ConfirmPin_VC animated:YES];
    }
